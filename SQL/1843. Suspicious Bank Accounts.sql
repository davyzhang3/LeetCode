...
with t1 as (
select t.account_id, sum(amount) monthsTotal, date_format(day, "%m") months, a.max_income
from transactions t
join accounts a on t.account_id=a.account_id
where type = "Creditor"
group by account_id, date_format(day, "%m")
having monthsTotal > max_income
order by t.account_id, months)

select distinct t2.account_id
from (
select *, lead(months) over (partition by account_id) next_month
from t1) t2
join transactions t on t2.account_id = t. account_id
where months = next_month -1
