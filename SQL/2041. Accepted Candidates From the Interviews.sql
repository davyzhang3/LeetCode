...
-- Write an SQL query to report the IDs of the candidates who have at least two years of experience and the sum of the score of their interview rounds is strictly greater or equal to 16.
...
from 
    (select 
    c.candidate_id,
    sum(r.score) over (partition by c.interview_id) as total_score
    from candidates c
    left join rounds r on c.interview_id = r.interview_id
    where c.years_of_exp>= 2) as sub
where total_score >= 16
